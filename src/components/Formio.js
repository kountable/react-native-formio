...
import {Text, ScrollView, StyleSheet, View, ActivityIndicator, NetInfo, AsyncStorage} from 'react-native';
...
loadForm(form) {
  ...
  if (form.settings.pdf) {
    form.components =  [{
      type: 'pdf',
      //source: form.settings.pdf.src + '.html?id=' + new Date().getTime(),
      html: '<script src="https://unpkg.com/formiojs@latest/dist/formio.embed.js?src='+ this.props.src +'"></script>'
    }]
  }
...
async onSubmit(event) {
  ...
  if (!isConnected) {
    let pendingSubmissionsStr = await AsyncStorage.getItem('pendingSubmissions')
    ...
    if (!pendingSubmissions || !isArray(pendingSubmissions)) {
      pendingSubmissions = []
    }

    snapshot.offline = true

    pendingSubmissions.push(snapshot)

    await AsyncStorage.setItem('pendingSubmissions', JSON.stringify(pendingSubmissions))

    const _this = this

    async function handleFirstConnectivityChange(isConnected) {
      if (!isConnected) {
        return
      }
      ...
      let savedPendingSubmissionsStr = await AsyncStorage.getItem('pendingSubmissions')
      let savedPendingSubmissions = []

      try {
        savedPendingSubmissions = JSON.parse(savedPendingSubmissionsStr)
      } catch(e) {}

      while (savedPendingSubmissions.length > 0) {
        let pendingSnapshot = savedPendingSubmissions.shift()

        try {
          _this.proceedSubmission(pendingSnapshot)
        } catch (e) {}

        await AsyncStorage.setItem('pendingSubmissions', JSON.stringify(savedPendingSubmissions))
      }
  ...
  this.proceedSubmission(snapshot)
}

proceedSubmission(snapshot) {
  let request;
  let method;
  let sub = snapshot.submission
  // Do the submit here.
  if (snapshot.action) {
    method = sub._id ? 'put' : 'post';
    request = Formiojs.request(snapshot.action, method, sub);
  }
  else if (this.formio) {
...
.catch((response) => {
  ...
  if (typeof this.props.onFormSubmit === 'function' && !snapshot.offline) {
